...
func (p *ProxyPostCommentHandler) Do() {
	...
	//正式调用Service层
	commentRes, err := sociality.PostComment(p.userId, p.videoId, p.commentId, p.actionType, p.commentText)
	if err != nil {
		p.SendError(err.Error())
		return
	}
...
func (p *ProxyPostCommentHandler) parseNum() error {
	...
	switch actionType {
	case sociality.CREATE:
		p.commentText = p.Query("comment_text")
	case sociality.DELETE:
...
func (p *ProxyPostCommentHandler) SendError(msg string) {
	p.JSON(http.StatusOK, PostCommentResponse{
		CommonResponse: models.CommonResponse{StatusCode: 1, StatusMsg: msg}, Response: &sociality.Response{}})
}

func (p *ProxyPostCommentHandler) SendOk(comment *sociality.Response) {
	p.JSON(http.StatusOK, PostCommentResponse{
